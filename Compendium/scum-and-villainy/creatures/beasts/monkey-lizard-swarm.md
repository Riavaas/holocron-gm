---
title: "Monkey-Lizard Swarm"
source: "SW5e Scum and Villainy"
source_file: "SW5e - Scum and Villainy - 20191105.pdf"
knowledge_type: "sw5e_compendium"
category: "creature_statblock"
book: "scum-and-villainy"
section: "Monkey-Lizard Swarm"
page_start: 98
page_end: 98
tags: ["beasts", "combatant", "cr-1-2", "creature", "monkey-lizard-swarm", "statblock", "swarm-of-tiny-beasts"]
status: "draft"
verbatim_risk: "low"
creature_name: "Monkey-Lizard Swarm"
creature_type: "swarm of tiny beasts"
size: "Medium"
alignment: "unaligned"
challenge_rating: "1/2"
xp: "100"
armor_class: "12"
hit_points: "24 (7d8-7)"
speed: "30 ft., climb 30 ft."
abilities: {"str": "6 (-2)", "dex": "14 (+2)", "con": "8 (-1)", "int": "3 (-4)", "wis": "12 (+1)", "cha": "6 (-2)"}
saving_throws: []
skills: ["Perception +5"]
damage_vulnerabilities: []
damage_resistances: ["energy", "kinetic"]
damage_immunities: []
condition_immunities: ["charmed", "frightened", "paralyzed", "petrified", "prone", "restrained", "stunned"]
senses: ["passive Perception 15"]
languages: []
traits: ["Pack Tactics", "Swarm"]
actions: ["Bite"]
reactions: []
legendary_actions: []
lair_actions: []
regional_effects: []
environment: []
faction: ["unknown"]
role: ["combatant"]
---

# Monkey-Lizard Swarm

## Source

* Book: SW5e Scum and Villainy
* Pages: 98-98
* Original section: Monkey-Lizard Swarm

## Quick Identity

* Type: Medium swarm of tiny beasts, unaligned
* Role: combatant
* Main danger: Bite
* Star Wars context: use as a unknown threat or encounter piece when that faction/context fits.

## Stat Summary

* Size / Type / Alignment: Medium swarm of tiny beasts, unaligned
* CR / XP: 1/2 / 100
* AC: 12
* HP: 24 (7d8-7)
* Speed: 30 ft., climb 30 ft.
* Key abilities: STR 6 (-2), DEX 14 (+2), CON 8 (-1), INT 3 (-4), WIS 12 (+1), CHA 6 (-2)
* Saves: none detected
* Skills: Perception +5
* Senses: passive Perception 15
* Languages: none detected

## Defenses

* Vulnerabilities: none detected
* Resistances: energy, kinetic
* Immunities: none detected
* Condition Immunities: charmed, frightened, paralyzed, petrified, prone, restrained, stunned

## Traits

Pack Tactics, Swarm

## Actions

Bite

## Reactions

None detected.

## Legendary / Lair / Regional

* Legendary actions: None detected.
* Lair actions: none detected
* Regional effects: none detected

## GM Use

* Use this creature when its CR, faction, and environment fit the scene.
* Combat role: combatant.
* Danger level: CR 1/2; verify exact mechanics on the cited source page.
* Run fast by tracking AC, HP, major defenses, and named actions first.

## Tactics

* Opener: choose the strongest listed action that fits range and positioning.
* Preferred targets: vulnerable or isolated characters if the role is skirmisher; frontline characters if the role is brute.
* Movement: use listed speed and terrain to pressure the party.
* Retreat/morale: droids follow mission logic; beasts retreat if badly hurt unless cornered.
* Synergies: pair with same faction/type allies from the indexes.

## Encounter Hooks

* Put the creature in a scene that highlights its role: combatant.
* Tie its arrival to faction pressure: unknown.
* Use the environment tags as encounter dressing: flexible.

## Loot / Rewards

GM suggestion, not official statblock data: salvage, trophies, local intel, or faction clues appropriate to the creature.

## Combat Tracker Hooks

* HP: 24 (7d8-7)
* AC: 12
* Initiative notes: use DEX 14 (+2).
* Conditions to apply: check traits/actions on source page.
* Recharges: check named actions on source page.
* Legendary actions: None detected.
* Lair action timing: none detected unless source page says otherwise.

## Related

* [Creature Index](../../indexes/creature-index.md)
* [CR Index](../../indexes/cr-index.md)
* [Type Index](../../indexes/type-index.md)
* [GM Cheatsheet](../../indexes/gm-cheatsheet.md)

## Search Tags

`beasts`, `combatant`, `cr-1-2`, `creature`, `monkey-lizard-swarm`, `statblock`, `swarm-of-tiny-beasts`
