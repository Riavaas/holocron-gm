---
title: "Force Trance"
source: "SW5e Player's Handbook"
source_file: "SW5e - Player's Handbook-avec compression.pdf"
knowledge_type: "sw5e_compendium"
category: "force_power"
chapter: "11-force-powers"
section: "Force Trance"
page_start: 249
page_end: 249
tags: ["force", "force-trance", "level-3", "power", "unconscious", "needs_review"]
status: "draft"
verbatim_risk: "low"
power_type: "force"
level: "3"
casting_time: "1 action"
range: "30 feet"
duration: "10 minutes"
concentration: false
save: ""
attack_roll: true
damage_types: []
conditions_inflicted: ["unconscious"]
classes_or_archetypes: []
---

# Force Trance

## Source

* Book: SW5e Player's Handbook
* Pages: 249-249
* Original section: Force Trance

## Quick Use

A 3 force power for condition control. Use the source page for exact effect text.

## Mechanics

* Level: 3
* Casting Time: 1 action
* Range: 30 feet
* Duration: 10 minutes
* Concentration: false
* Save: 
* Attack Roll: false
* Damage: 
* Conditions: unconscious
* Area: 
* Scaling / Higher-Level Effect: detected; check source text

## GM Notes

* Confirm the casting time, range, target, and duration before resolving the power.
* Use the exact source page for full effect text and edge cases.
* If metadata is incomplete, treat this card as a lookup pointer and review the source.

## Combat Tracker Hooks

* Condition to apply: unconscious
* Concentration to track: false
* Damage to roll: 
* Save DC: 
* Attack roll: false
* Target/area: 30 feet 
* Duration: 10 minutes

## Related

* [Power Duration](../../rules-cards/power-duration.md)
* [Casting Time](../../rules-cards/casting-time.md)
* [Power Targets](../../rules-cards/power-targets.md)
* [Unconscious](../../conditions/unconscious.md)

## Search Tags

`force`, `force-trance`, `level-3`, `power`, `unconscious`
