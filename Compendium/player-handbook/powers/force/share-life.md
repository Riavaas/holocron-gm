---
title: "Share Life"
source: "SW5e Player's Handbook"
source_file: "SW5e - Player's Handbook-avec compression.pdf"
knowledge_type: "sw5e_compendium"
category: "force_power"
chapter: "11-force-powers"
section: "Share Life"
page_start: 263
page_end: 263
tags: ["force", "level-3", "necrotic", "power", "share-life"]
status: "draft"
verbatim_risk: "low"
power_type: "force"
level: "3"
casting_time: "1 action"
range: "30 feet"
duration: "Instantaneous"
concentration: false
save: ""
attack_roll: false
damage_types: ["necrotic"]
conditions_inflicted: []
classes_or_archetypes: []
---

# Share Life

## Source

* Book: SW5e Player's Handbook
* Pages: 263-263
* Original section: Share Life

## Quick Use

A 3 force power for damage. Use the source page for exact effect text.

## Mechanics

* Level: 3
* Casting Time: 1 action
* Range: 30 feet
* Duration: Instantaneous
* Concentration: false
* Save: 
* Attack Roll: false
* Damage: 1d8, 4d8, necrotic
* Conditions: 
* Area: 
* Scaling / Higher-Level Effect: detected; check source text

## GM Notes

* Confirm the casting time, range, target, and duration before resolving the power.
* Use the exact source page for full effect text and edge cases.
* If metadata is incomplete, treat this card as a lookup pointer and review the source.

## Combat Tracker Hooks

* Condition to apply: 
* Concentration to track: false
* Damage to roll: 1d8, 4d8, necrotic
* Save DC: 
* Attack roll: false
* Target/area: 30 feet 
* Duration: Instantaneous

## Related

* [Power Duration](../../rules-cards/power-duration.md)
* [Casting Time](../../rules-cards/casting-time.md)
* [Power Targets](../../rules-cards/power-targets.md)

## Search Tags

`force`, `level-3`, `necrotic`, `power`, `share-life`
