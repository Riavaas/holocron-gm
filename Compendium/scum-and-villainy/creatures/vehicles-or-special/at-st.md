---
title: "AT-ST"
source: "SW5e Scum and Villainy"
source_file: "SW5e - Scum and Villainy - 20191105.pdf"
knowledge_type: "sw5e_compendium"
category: "creature_statblock"
book: "scum-and-villainy"
section: "AT-ST"
page_start: 65
page_end: 65
tags: ["at-st", "combatant", "construct", "cr-11", "creature", "statblock", "vehicles-or-special"]
status: "draft"
verbatim_risk: "low"
creature_name: "AT-ST"
creature_type: "construct"
size: "Huge"
alignment: "unaligned"
challenge_rating: "11"
xp: "7,200"
armor_class: "17 (armor plating)"
hit_points: "168 (16d12+64)"
speed: "40 ft."
abilities: {"str": "23 (+6)", "dex": "16 (+3)", "con": "19 (+4)", "int": "10 (+0)", "wis": "14 (+2)", "cha": "7 (-2)"}
saving_throws: ["Str +10", "Con +8"]
skills: []
damage_vulnerabilities: ["ion"]
damage_resistances: ["psychic"]
damage_immunities: ["poison", "necrotic"]
condition_immunities: ["poisoned", "disease", "frightened", "petrified", "restrained", "incapacitated", "stunned", "prone", "blinded", "charmed"]
senses: []
languages: []
traits: ["Piloted", "Circuitry"]
actions: ["Multiattack", "Medium Repeating Blasters"]
reactions: []
legendary_actions: []
lair_actions: []
regional_effects: []
environment: []
faction: ["unknown"]
role: ["combatant"]
---

# AT-ST

## Source

* Book: SW5e Scum and Villainy
* Pages: 65-65
* Original section: AT-ST

## Quick Identity

* Type: Huge construct, unaligned
* Role: combatant
* Main danger: Multiattack, Medium Repeating Blasters
* Star Wars context: use as a unknown threat or encounter piece when that faction/context fits.

## Stat Summary

* Size / Type / Alignment: Huge construct, unaligned
* CR / XP: 11 / 7,200
* AC: 17 (armor plating)
* HP: 168 (16d12+64)
* Speed: 40 ft.
* Key abilities: STR 23 (+6), DEX 16 (+3), CON 19 (+4), INT 10 (+0), WIS 14 (+2), CHA 7 (-2)
* Saves: Str +10, Con +8
* Skills: none detected
* Senses: none detected
* Languages: none detected

## Defenses

* Vulnerabilities: ion
* Resistances: psychic
* Immunities: poison, necrotic
* Condition Immunities: poisoned, disease, frightened, petrified, restrained, incapacitated, stunned, prone, blinded, charmed

## Traits

Piloted, Circuitry

## Actions

Multiattack, Medium Repeating Blasters

## Reactions

None detected.

## Legendary / Lair / Regional

* Legendary actions: None detected.
* Lair actions: none detected
* Regional effects: none detected

## GM Use

* Use this creature when its CR, faction, and environment fit the scene.
* Combat role: combatant.
* Danger level: CR 11; verify exact mechanics on the cited source page.
* Run fast by tracking AC, HP, major defenses, and named actions first.

## Tactics

* Opener: choose the strongest listed action that fits range and positioning.
* Preferred targets: vulnerable or isolated characters if the role is skirmisher; frontline characters if the role is brute.
* Movement: use listed speed and terrain to pressure the party.
* Retreat/morale: droids follow mission logic; beasts retreat if badly hurt unless cornered.
* Synergies: pair with same faction/type allies from the indexes.

## Encounter Hooks

* Put the creature in a scene that highlights its role: combatant.
* Tie its arrival to faction pressure: unknown.
* Use the environment tags as encounter dressing: flexible.

## Loot / Rewards

GM suggestion, not official statblock data: salvage, trophies, local intel, or faction clues appropriate to the creature.

## Combat Tracker Hooks

* HP: 168 (16d12+64)
* AC: 17 (armor plating)
* Initiative notes: use DEX 16 (+3).
* Conditions to apply: check traits/actions on source page.
* Recharges: check named actions on source page.
* Legendary actions: None detected.
* Lair action timing: none detected unless source page says otherwise.

## Related

* [Creature Index](../../indexes/creature-index.md)
* [CR Index](../../indexes/cr-index.md)
* [Type Index](../../indexes/type-index.md)
* [GM Cheatsheet](../../indexes/gm-cheatsheet.md)

## Search Tags

`at-st`, `combatant`, `construct`, `cr-11`, `creature`, `statblock`, `vehicles-or-special`
