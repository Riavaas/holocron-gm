---
title: "Destroy Tech"
source: "SW5e Player's Handbook"
source_file: "SW5e - Player's Handbook-avec compression.pdf"
knowledge_type: "sw5e_compendium"
category: "tech_power"
chapter: "12-tech-powers"
section: "Destroy Tech"
page_start: 276
page_end: 276
tags: ["destroy-tech", "intelligence-save", "ion", "level-3", "power", "tech"]
status: "draft"
verbatim_risk: "low"
power_type: "tech"
level: "3"
casting_time: "1 action"
range: "120 feet"
duration: "Instantaneous"
concentration: false
save: "Intelligence"
attack_roll: true
damage_types: ["ion"]
conditions_inflicted: []
classes_or_archetypes: []
---

# Destroy Tech

## Source

* Book: SW5e Player's Handbook
* Pages: 276-276
* Original section: Destroy Tech

## Quick Use

A 3 tech power for damage, attack roll resolution, Intelligence save pressure. Use the source page for exact effect text.

## Mechanics

* Level: 3
* Casting Time: 1 action
* Range: 120 feet
* Duration: Instantaneous
* Concentration: false
* Save: Intelligence
* Attack Roll: true
* Damage: 1d8, 4d8, ion
* Conditions: 
* Area: 
* Scaling / Higher-Level Effect: detected; check source text

## GM Notes

* Confirm the casting time, range, target, and duration before resolving the power.
* Use the exact source page for full effect text and edge cases.
* If metadata is incomplete, treat this card as a lookup pointer and review the source.

## Combat Tracker Hooks

* Condition to apply: 
* Concentration to track: false
* Damage to roll: 1d8, 4d8, ion
* Save DC: Intelligence
* Attack roll: true
* Target/area: 120 feet 
* Duration: Instantaneous

## Related

* [Power Duration](../../rules-cards/power-duration.md)
* [Power Save Dc](../../rules-cards/power-save-dc.md)
* [Power Targets](../../rules-cards/power-targets.md)

## Search Tags

`destroy-tech`, `intelligence-save`, `ion`, `level-3`, `power`, `tech`
