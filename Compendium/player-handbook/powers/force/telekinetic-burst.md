---
title: "Telekinetic Burst"
source: "SW5e Player's Handbook"
source_file: "SW5e - Player's Handbook-avec compression.pdf"
knowledge_type: "sw5e_compendium"
category: "force_power"
chapter: "11-force-powers"
section: "Telekinetic Burst"
page_start: 266
page_end: 266
tags: ["concentration", "constitution-save", "force", "level-6", "power", "prone", "telekinetic-burst"]
status: "draft"
verbatim_risk: "low"
power_type: "force"
level: "6"
casting_time: "1 action"
range: "Self (60-foot line)"
duration: "Concentration, up to 1 minute"
concentration: true
save: "Constitution"
attack_roll: true
damage_types: ["force"]
conditions_inflicted: ["prone"]
classes_or_archetypes: []
---

# Telekinetic Burst

## Source

* Book: SW5e Player's Handbook
* Pages: 266-266
* Original section: Telekinetic Burst

## Quick Use

A 6 force power for damage, condition control, Constitution save pressure, ongoing concentration. Use the source page for exact effect text.

## Mechanics

* Level: 6
* Casting Time: 1 action
* Range: Self (60-foot line)
* Duration: Concentration, up to 1 minute
* Concentration: true
* Save: Constitution
* Attack Roll: false
* Damage: 2d6, 8d6, force
* Conditions: prone
* Area: 60-foot line
* Scaling / Higher-Level Effect: detected; check source text

## GM Notes

* Confirm the casting time, range, target, and duration before resolving the power.
* Use the exact source page for full effect text and edge cases.
* If metadata is incomplete, treat this card as a lookup pointer and review the source.

## Combat Tracker Hooks

* Condition to apply: prone
* Concentration to track: true
* Damage to roll: 2d6, 8d6, force
* Save DC: Constitution
* Attack roll: false
* Target/area: Self (60-foot line)  / 60-foot line
* Duration: Concentration, up to 1 minute

## Related

* [Concentration](../../rules-cards/concentration.md)
* [Power Save Dc](../../rules-cards/power-save-dc.md)
* [Power Targets](../../rules-cards/power-targets.md)
* [Prone](../../conditions/prone.md)

## Search Tags

`concentration`, `constitution-save`, `force`, `level-6`, `power`, `prone`, `telekinetic-burst`
