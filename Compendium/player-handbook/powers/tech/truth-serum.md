---
title: "Truth Serum"
source: "SW5e Player's Handbook"
source_file: "SW5e - Player's Handbook-avec compression.pdf"
knowledge_type: "sw5e_compendium"
category: "tech_power"
chapter: "12-tech-powers"
section: "Truth Serum"
page_start: 301
page_end: 301
tags: ["constitution-save", "level-2", "power", "tech", "truth-serum"]
status: "draft"
verbatim_risk: "low"
power_type: "tech"
level: "2"
casting_time: "1 action"
range: "Touch"
duration: "10 minutes"
concentration: false
save: "Constitution"
attack_roll: false
damage_types: []
conditions_inflicted: []
classes_or_archetypes: []
---

# Truth Serum

## Source

* Book: SW5e Player's Handbook
* Pages: 301-301
* Original section: Truth Serum

## Quick Use

A 2 tech power for Constitution save pressure. Use the source page for exact effect text.

## Mechanics

* Level: 2
* Casting Time: 1 action
* Range: Touch
* Duration: 10 minutes
* Concentration: false
* Save: Constitution
* Attack Roll: false
* Damage: 
* Conditions: 
* Area: 
* Scaling / Higher-Level Effect: 

## GM Notes

* Confirm the casting time, range, target, and duration before resolving the power.
* Use the exact source page for full effect text and edge cases.
* If metadata is incomplete, treat this card as a lookup pointer and review the source.

## Combat Tracker Hooks

* Condition to apply: 
* Concentration to track: false
* Damage to roll: 
* Save DC: Constitution
* Attack roll: false
* Target/area: Touch 
* Duration: 10 minutes

## Related

* [Power Duration](../../rules-cards/power-duration.md)
* [Power Save Dc](../../rules-cards/power-save-dc.md)
* [Power Targets](../../rules-cards/power-targets.md)

## Search Tags

`constitution-save`, `level-2`, `power`, `tech`, `truth-serum`
