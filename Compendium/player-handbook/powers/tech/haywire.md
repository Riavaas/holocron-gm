---
title: "Haywire"
source: "SW5e Player's Handbook"
source_file: "SW5e - Player's Handbook-avec compression.pdf"
knowledge_type: "sw5e_compendium"
category: "tech_power"
chapter: "12-tech-powers"
section: "Haywire"
page_start: 283
page_end: 283
tags: ["haywire", "intelligence-save", "level-at-will", "lightning", "power", "tech"]
status: "draft"
verbatim_risk: "low"
power_type: "tech"
level: "at-will"
casting_time: "1 action"
range: "30 feet"
duration: "Instantaneous"
concentration: false
save: "Intelligence"
attack_roll: false
damage_types: ["lightning"]
conditions_inflicted: []
classes_or_archetypes: []
---

# Haywire

## Source

* Book: SW5e Player's Handbook
* Pages: 283-283
* Original section: Haywire

## Quick Use

An at-will tech power for damage, Intelligence save pressure. Use the source page for exact effect text.

## Mechanics

* Level: at-will
* Casting Time: 1 action
* Range: 30 feet
* Duration: Instantaneous
* Concentration: false
* Save: Intelligence
* Attack Roll: false
* Damage: 1d6, 2d6, 3d6, 4d6, lightning
* Conditions: 
* Area: 
* Scaling / Higher-Level Effect: 

## GM Notes

* Confirm the casting time, range, target, and duration before resolving the power.
* Use the exact source page for full effect text and edge cases.
* If metadata is incomplete, treat this card as a lookup pointer and review the source.

## Combat Tracker Hooks

* Condition to apply: 
* Concentration to track: false
* Damage to roll: 1d6, 2d6, 3d6, 4d6, lightning
* Save DC: Intelligence
* Attack roll: false
* Target/area: 30 feet 
* Duration: Instantaneous

## Related

* [Power Duration](../../rules-cards/power-duration.md)
* [Power Save Dc](../../rules-cards/power-save-dc.md)
* [Power Targets](../../rules-cards/power-targets.md)

## Search Tags

`haywire`, `intelligence-save`, `level-at-will`, `lightning`, `power`, `tech`
